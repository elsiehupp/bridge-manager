...
export class WhoamiAsmuxData {
	LoginToken: string; // `json:"login_token"`
}

export class WhoamiUser {
...
export const newRequest = (baseDomain, token, method, path: string) *http.Request {
	req = &http.Request{
		URL: &url.URL{
			Scheme: "https",
...
export const encodeContent = (into *http.Request, body any) error {
...
export const doRequest = (req *http.Request, reqData, resp any) (err error) {
	if reqData != nil {
...
export class ReqPostBridgeState {
	StateEvent   status.BridgeStateEvent // `json:"stateEvent"`
	Reason: string; // `json:"reason"`
	Info         map[string]any // `json:"info"`
	IsSelfHosted: bool // `json:"isSelfHosted"`
...
export const DeleteBridge = (domain, bridgeName, token: string) error {
	req = newRequest(domain, token, http.MethodDelete, fmt.Sprintf("/bridge/%s", bridgeName))
	return doRequest(req, nil, nil)
}

export const PostBridgeState = (domain, username, bridgeName, asToken: string, data ReqPostBridgeState) error {
...
export const Whoami = (baseDomain, token: string) (resp *RespWhoami, err error) {
